...
public class AuthenticationViaFormAction {
    ...
    public final Event submit(final RequestContext context, final Credential credential,
        ...
        boolean auth=true;
        int nerrcount = session.getAttribute("errcount")==null? 0:(int)session.getAttribute("errcount");
        if( nerrcount>=3)
        {
            ...
            {
                submitAuthcode = submitAuthcode.toLowerCase();
            }
            //if(!StringUtils.hasText(submitAuthcode) || !StringUtils.hasText(authcode)){
            if(submitAuthcode==null || submitAuthcode.isEmpty() || authcode==null || authcode.isEmpty()) {
                //throw new NullAuthcodeAuthenticationException();
                //populateErrorsInstance(new NullAuthcodeAuthenticationException(),messageContext);
                auth = false;
            }else if(!submitAuthcode.equals(authcode))
            {
                auth = false;
            }
        }
        ...
        if (auth && !checkLoginTicketIfExists(context)) {
            ret = returnInvalidLoginTicketEvent(context, messageContext);
        }

        if (auth && isRequestAskingForServiceTicket(context)) {
            ret = grantServiceTicket(context, credential);
        }

        if(auth)
        {
            ret = createTicketGrantingTicket(context, credential, messageContext);
        }
        final Service service = WebUtils.getService(context);
        if(ret.getId().equals(AUTHENTICATION_FAILURE) || ret.getId().equals(ERROR))
        {
            nerrcount++;
            session.setAttribute("errcount",nerrcount);
        }else
        {
...
//            Map<String,Object> attrs = tgt.getAuthentication().getPrincipal().getAttributes();
            final Service service = WebUtils.getService(context);
...
//            {
                ...
                if (!registeredService.getAccessStrategy().doPrincipalAttributesAllowServiceAccess(principalAttrs)) {
                    ...
                    String servicet = registeredService.getAccessStrategy().getUnauthorizedRedirectUrl().toString();
                    String code = String.format("service.not.authorized.missing.attr.%s",servicet);
...
//                messageContext.addMessage(new MessageBuilder().error().code("error.authentication.authcode.bad").build());
//                return newEvent(ERROR);
//            }else if(attrs.get("role").equals("normal"))
//            {
                //普通用户不能登入管理平台
//                messageContext.addMessage(new MessageBuilder().error().code("error.authentication.authcode.bad").build());
//                return newEvent(ERROR);
...
//            }
     ...
     * @throws Exception
     */
    public final String validatorCode(final RequestContext context, final Credentials credential, final MessageContext messageContext)  {
        final HttpServletRequest request = WebUtils.getHttpServletRequest(context);
        HttpSession session = request.getSession();
        String authcode = (String)session.getAttribute(com.google.code.kaptcha.Constants.KAPTCHA_SESSION_KEY);
        session.removeAttribute(com.google.code.kaptcha.Constants.KAPTCHA_SESSION_KEY);
        ...
        logger.error(String.format("submit authcode:%s $$$$$$$$$$$$$$$$$$$$$$",authcode));
        logger.error(String.format("submit authcode:%s $$$$$$$$$$$$$$$$$$$$$$",upc.getUsername()));
        logger.error(String.format("submit authcode:%s $$$$$$$$$$$$$$$$$$$$$$",upc.getAuthcode()));
